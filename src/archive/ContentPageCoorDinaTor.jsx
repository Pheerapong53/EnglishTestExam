...
function ContentPageCoorDinator() {
  ...
  //filter member by usertypeid = usr03 and select data to show in datagrid
  const members = filter("USR03", MemberLists);
  //console.log(members);
  ...
  const toDeleteMember = (clickedMember) => {
    ...
    // const delId = delData[0].pers_id + "-" + "USR03";
    const delId = `${delData[0].pers_id}-USR03`;
...
export default ContentPageCoorDinator;
